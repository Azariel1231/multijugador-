class Student {
    constructor(){
        ...
        this.nameInput = createInput("").attribute("placehondel", "Nombre");
    ...
    getToken(word){
        ...
        httpGet(url, "json", false, response =>{
            if(response.success) {
                this.login(response.toker,word);
            } else {
                swal({
    ...
    login(toker, secret_word){
        fireAuth.singInWithCustomToken(token).catch(function(error) {
